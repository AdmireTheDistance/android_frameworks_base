    ...
    public boolean requestPowerState(DisplayPowerRequest request,
        ...
        synchronized (mLock) {
            ...
            if (changed && !mPendingRequestChangedLocked) {
            	if ((mKeyguardService != null && !mKeyguardService.isShowing()) && request.policy == DisplayPowerRequest.POLICY_OFF) {
            		boolean seeThrough = Settings.System.getInt(mContext.getContentResolver(), Settings.System.LOCKSCREEN_SEE_THROUGH, 0) == 1;
            		if (seeThrough) {
            			...
            			boolean reverse = rotation == Surface.ROTATION_90 || rotation == Surface.ROTATION_270;
            			Bitmap bmp = SurfaceControl.screenshot(new Rect(), reverse ? point.y : point.x, reverse ? point.x : point.y, 0, 22000, false, Surface.ROTATION_0);
            			if (bmp != null) {
            				Bitmap tmpBmp = bmp;
